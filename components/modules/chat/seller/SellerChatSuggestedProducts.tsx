...
interface SellerChatSuggestedProductsProps {
  suggestions: any[];
}

/**
 * Renders the list of suggested/alternative products under a requested product row.
 */
const SellerChatSuggestedProducts: React.FC<SellerChatSuggestedProductsProps> = ({ suggestions }) => {
  const { currency } = useAuth();

  const visible = suggestions.filter((s: any) => (s.quantity ?? 0) > 0);
          ...
          >
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="relative h-7 w-7 flex-shrink-0 overflow-hidden rounded border border-border bg-card">
                <Image src={imageUrl} alt={p?.productName || "Product"} fill className="object-cover" />
              </div>
              <div className="min-w-0 flex-1">
                <span className="truncate block text-[11px]">{p?.productName || "-"}</span>
                {s.quantity && s.quantity > 0 && (
                  <span className="text-[9px] text-muted-foreground">Qty: {s.quantity}</span>
                )}
              </div>
            </div>
            ...
            <div className="text-center text-[10px] text-muted-foreground">
              <span>{totalPrice ? `${currency.symbol}${totalPrice}` : "-"}</span>
            </div>
...
export default SellerChatSuggestedProducts;
