...
const Sidebar: React.FC<SidebarProps> = ({ notificationCount }) => {
		...
		const baseItems: MenuItem[] = [
			{
				...
				path: "/my-accounts",text-primary h-5 w-5" />,
        label: t("my_accounts"),
        translationKey: "my_accounts",
        shortLabel: getShortLabel("my_accounts"),
        path: "/my-accounts",
    ...
    if (currentTradeRole === "BUYER") {
      baseItems.push(
        {
          icon: <ShoppingBagIcon className="text-info h-5 w-5" />,
          label: t("my_orders"),
        ...
        {
          icon: <FileSearchIcon className="text-info h-5 w-5" />,
        ...
        {
          icon: <ShoppingCartIcon className="text-warning h-5 w-5" />,
          label: t("cart"),
          translationKey: "cart",
        ...
        {
          icon: <StoreIcon className="text-destructive h-5 w-5" />,
          label: t("wishlist"),
    ...
    if (currentTradeRole === "FREELANCER") {
      baseItems.push(
        {
          icon: <PackageIcon className="text-primary h-5 w-5" />,
          label: t("my_products"),
        ...
        {
          icon: <WrenchIcon className="text-success h-5 w-5" />,
          label: t("my_services"),
        ...
        {
          icon: <UsersIcon className="text-info h-5 w-5" />,
        ...
        {
          icon: <MessageCircleIcon className="text-info h-5 w-5" />,
          label: t("messages"),
        ...
        {
          icon: <BarChart3Icon className="text-success h-5 w-5" />,
          label: t("analytics"),
    ...
    if (currentTradeRole === "COMPANY") {
      baseItems.push(
        ...
        {
          icon: <PackageIcon className="text-info h-5 w-5" />,
    ...
    baseItems.push(
      {
        icon: <UserCheckIcon className="text-primary h-5 w-5" />,
        label: t("profile"),
      ...
      {
        icon: <WalletIcon className="text-warning h-5 w-5" />,
        label: t("my_wallet"),
      ...
      {
        icon: <SettingsIcon className="text-muted-foreground h-5 w-5" />,
        label: t("settings"),
      ...
      {
        icon: <LogOutIcon className="text-destructive h-5 w-5
				onClick: async () => {
					try {
		...
		<>
			...
			{accessToken && (
				<aside
					...
					className={cn(
						"bg-sidebar fixed top-0 z-[70] h-full shadow-xl",
                ...
                Total: ~96px md:~113px lg:~126px (increased slightly) */}
						<div
							className={cn(
								"bg-sidebar-primary relative flex items-center justify-between overflow-hidden",
						...
						>
							...
							<div className="absolute inset-0 opacity-10">
								<div className="bg-sidebar-primary-foreground absolute top-0 right-0 -mt-16 -mr-16 h-32 w-32 rounded-full"></div>
								<div className="bg-sidebar-primary-foreground absolute bottom-0 left-0 -mb-12 -ml-12 h-24 w-24 rounded-full"></div>
							</div>
							{isHovered || isOpen ? (
								<div className="text-sidebar-primary-foreground relative z-10 flex w-full items-center justify-center">
									...
									<button
										onClick={closeSidebar}
										className={cn(
											"hover:bg-sidebar-primary-foreground/20 absolute flex h-8 w-8 items-center justify-center rounded-lg transition-colors md:hidden",
											langDir === "rtl" ? "left-4" : "right-4",
										)}
										aria-label="Close menu"
									>
										<XIcon className="text-sidebar-primary-foreground h-5 w-5" />
									</button>
								</div>
							) : (
								<div className="text-sidebar-primary-foreground relative z-10 flex w-full items-center justify-center">
						...
						<div className="scrollbar-hide bg-sidebar flex-1 overflow-x-visible overflow-y-auto">
									...
									return (
										<div
											...
											className={cn(
												...
												isHovered || isOpen
													? "hover:bg-sidebar-accent/10 mx-2 justify-between rounded-lg px-4 py-2 hover:shadow-sm"
													: "hover:bg-sidebar-accent/10 mx-1 min-h-[64px] flex-col justify-center rounded-lg px-1 py-2.5",
												isActive &&
													(isHovered || isOpen) &&
													"bg-sidebar-accent/15 shadow-sm",
												isActive &&
													!(isHovered || isOpen) &&
													"bg-sidebar-accent/20 rounded-lg",
												item.isLogout &&
													"border-sidebar-border mt-3 border-t-2 pt-3",
												langDir === "rtl" &&
													(isHovered || isOpen) &&
													"flex-row-reverse",
											)}
										...
										>
											...
											<div
												className={cn(
													"bg-sidebar-primary absolute top-1/2 h-0 w-1 -translate-y-1/2 opacity-0 transition-all duration-200 group-hover:h-8 group-hover:opacity-100",
													langDir === "rtl"
											...
											>
												<div
													className={cn(
														...
														!(isHovered || isOpen) &&
															"group-hover:bg-sidebar-accent/10 group-hover:scale-110",
													)}
												>
													<div
														className={cn(
													...
													{item.isLogout && (
														<div className="border-sidebar bg-destructive absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 shadow-sm"></div>
													)}
												</div>
												...
												{(isHovered || isOpen) && (
													<div className="min-w-0 flex-1">
														...
														{item.subLabel && (
															<div className="text-sidebar-foreground/60 mt-0.5 text-xs whitespace-nowrap">
																{item.subLabel}
															</div>
														)}
													</div>
											...
											{(isHovered || isOpen) && !item.isLogout && (
												<ChevronRightIcon
													className={cn(
														"text-sidebar-foreground/50 group-hover:text-sidebar-primary h-4 w-4 shrink-0 transition-transform duration-200",
														langDir === "rtl"
						...
						{(isHovered || isOpen) && (
							<div className="border-sidebar-border bg-sidebar border-t px-4 py-3">
								<div className="text-sidebar-foreground/60 flex items-center justify-center gap-2 text-xs">
