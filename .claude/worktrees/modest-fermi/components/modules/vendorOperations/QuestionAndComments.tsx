...
const QuestionAndAnswers: React.FC<QuestionAndAnswersProps> = ({
  ...
  return (
    <div className="w-full border-r border-solid border-border lg:w-[67%]">
      <div className="flex min-h-[55px] w-full items-center justify-between border-b border-solid border-border px-[10px] py-[10px] text-base font-normal text-foreground" dir={langDir}>
      ...
      <div className="flex w-full border-t-2 border-border py-5">
        <div className="w-full space-y-3">
          ...
          {!questionQuery?.isLoading && !memoizedQuestions?.length ? (
            <p className="text-center text-sm font-normal text-muted-foreground" dir={langDir} translate="no">
              {t("no_data_found")}
            </p>
          ) : null}
