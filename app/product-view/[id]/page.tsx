...
import { useCartListByDevice, useCartListByUserId, useUpdateCartByDevice, useUpdateCartWithLogin, useDeleteCartItem } from "@/apis/queries/cart.queries";
...
import {
  ...
  Zap, Check, X, Copy, Eye, ChevronDown, Trash2,
} from "lucide-react";
...
export default function ProductViewPage() {
  ...
  return (
    <>
      ...
      {/* Fixed Sidebar Cart - Desktop Only (matching /trending style) */}
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="sticky top-0 z-10 border-b border-border bg-card px-4 pt-4 pb-3 text-center">
                ...
                <button
                  ...
                  className="mt-3 flex w-full items-center justify-center space-x-1.5 rounded-lg bg-warning px-3 py-2 text-xs font-medium text-foreground shadow-sm transition-colors hover:bg-warning"
                >
                  <Package className="h-3 w-3" />
                  <span>{t("go_to_cart") || "Go To Cart"}</span>
                </button>
              </div>
              ...
              <div className="scrollbar-hide flex-1 overflow-y-auto px-4 pt-3 pb-4">
                <div className="space-y-3">
                  {cartList.map((cartItem: any) => {
                    ...
                    const itemImage = cartItem.productPriceDetails?.productPrice_product?.productImages?.[0]?.image || cartItem.productAllImage?.[0]?.image || null;
                    const itemPrice = Number(cartItem.offerPrice || cartItem.productPrice || 0);

                    return (
                      <div key={cartItem.id} className="space-y-2 text-center">
                        {/* Product Image */}
                        <div className="flex justify-center">
                          <Link href={`/trending/${cartItem.productId}`}
                        ...
                        {/* Price */}
                        <p className="text-sm font-semibold text-foreground">${(itemPrice * itemQuantity).toFixed(2)}</p>
                        {/* Quantity */}
                        <div className="flex items-center justify-center">
                          <div className="flex items-center overflow-hidden rounded-md border-2 border-yellow-400">
                            <button
                              onClick={async () => {
                                if (itemQuantity > 1) {
                                  if (haveAccessToken) await updateCartWithLogin.mutateAsync({ productPriceId: cartItem.productPriceId, quantity: itemQuantity - 1 });
                                  else await updateCartByDevice.mutateAsync({ productPriceId: cartItem.productPriceId, quantity: itemQuantity - 1, deviceId });
                                  queryClient.invalidateQueries({ queryKey: haveAccessToken ? ["cart-by-user-id"] : ["cart-list-by-device-id"] });
                                } else {
                                  await deleteCartItem.mutateAsync({ cartId: cartItem.id });
                                  queryClient.invalidateQueries({ queryKey: haveAccessToken ? ["cart-by-user-id"] : ["cart-list-by-device-id"] });
                                }
                              }}
                              className="px-1.5 py-1 transition-colors hover:bg-warning/5"
                            >
